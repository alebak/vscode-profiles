...
IF "%~1" == "-h" GOTO help
IF "%~1" == "--go" GOTO go
IF "%~1" == "--js" GOTO javascript
IF "%~1" == "--php" GOTO php
IF "%~1" == "--python" GOTO python
IF "%~1" == "--ts" GOTO typescript
...
:loadprofile
	code --extensions-dir %USERPROFILE%/.vscode-profiles/%~1/exts --user-data-dir %USERPROFILE%/.vscode-profiles/%~1/data
	GOTO exit
...
:help
	...
	ECHO Usage: vscode.bat [options] %n%
	...
	ECHO Options &
	ECHO %t%--go     %t%%t%Go profile.
	ECHO %t%--js     %t%%t%JavaScript profile.
	ECHO %t%--php    %t%%t%PHP profile.
	ECHO %t%--python %t%%t%Python profile.
	ECHO %t%--ts     %t%%t%TypeScript profile.%n%
